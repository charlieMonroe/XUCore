...
open class XUManagedDataContext {
	...
	public init(persistentStoreURL: URL, concurrencyType: NSManagedObjectContextConcurrencyType = .mainQueueConcurrencyType) {
		self.managedObjectContext = NSManagedObjectContext(concurrencyType: concurrencyType)

		guard let objectModel = NSManagedObjectModel.mergedModel(from: [ XUMainBundle ]) else {
