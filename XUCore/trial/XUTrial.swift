...
open class XUTrial: NSObject {
	...
	@objc fileprivate func _warnAboutNoInternetConnection() {
		let alert = NSAlert()
		alert.messageText = XULocalizedFormattedString("%@ couldn't connect to the Internet. The application will exit in one hour.", ProcessInfo.processInfo.processName, inBundle: XUCoreBundle)
		alert.informativeText = XULocalizedFormattedString("%@ requires connection to the Internet to continue the trial properly.", ProcessInfo.processInfo.processName, inBundle: XUCoreBundle)
	...
	open func showTrialAlertWithMessage(_ message: String) {
		let alert = NSAlert()
		...
		alert.addButton(withTitle: XULocalizedString("Continue", inBundle: XUCoreBundle))
		alert.addButton(withTitle: XULocalizedString("Purchase...", inBundle: XUCoreBundle))
	...
	/// ID may be nil.
	open func trialExpiredWithTrialID(_ trialID: String?) {
		...
		alert.addButton(withTitle: XULocalizedString("Purchase...", inBundle: XUCoreBundle))
		alert.addButton(withTitle: XULocalizedString("I'm Still Not Sure", inBundle: XUCoreBundle))
