...
public final class XUExceptionHandler: NSObject, XUFatalErrorObserver {
	...
	public func fatalErrorDidOccur(with reason: String) {
		...
		DispatchQueue.main.syncOrNow {
			if let provider = XUAppSetup.applicationStateProvider {
				XUExceptionCatcher.perform({
					stackTraceString.prepend(with: provider.provideApplicationState() + "\n\n")
				}, withCatchHandler: { (exception) in
					stackTraceString.prepend(with: "Failed to get application state - fetching it resulted in an exception \(exception).\n\n")
				})
			}

			
			XUExceptionReporter.showReporter(for: exception, thread: thread, queue: queue, andStackTrace: stackTraceString)
		}
	}
